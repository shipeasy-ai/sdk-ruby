...
Shipeasy doc standard (experiment-platform `23-sdk-dx-standard.md`, which extends
`21-tooling-monorepo.md` §A4.4 and doc 22) so the same structure exists in every
Shipeasy SDK repo and can be consumed by tooling.
...
```
https://shipeasy-ai.github.io/sdk-ruby/manifest.json
https://shipeasy-ai.github.io/sdk-ruby/pages/<page>.md
https://shipeasy-ai.github.io/sdk-ruby/snippets/<group>/<leaf>.md
...
this folder: end users (rendered on github.com), the Shipeasy CLI/MCP `docs`
op (raw fetch from Pages), and the central docs portal (`apps/ui`/`apps/docs`,
which renders these files into a themed site).
...
- `sdk` — the registry name (`"ruby"`).
- `placeholders` — every `{{TOKEN}}` used in snippets (callers substitute these).
  Currently: `FLAG_KEY`, `CONFIG_KEY`, `KILLSWITCH_KEY`, `EXPERIMENT_KEY`,
  `EVENT_NAME`, `SUCCESS_EVENT`, `PROFILE`. The identifier is the stable **key**
  (schema field `name`); "slug" is only for projects. Add a token here when you
  introduce it.
...
- `pages` — map of page key → path. The **keys are a fixed vocabulary shared
  ...
  advanced`. Don't rename keys; the SDK may add extra ones.
- `snippets` — nested `{ group: { leaf: path } }`. Current groups: `release`
...
SDK's real API**. Written around `Shipeasy.configure` + `Shipeasy::Client.new(user)`
— never the `Engine` (see the repo-root `CLAUDE.md`).

### `snippets/<group>/<leaf>.md`

Minimal copy-paste examples. Conventions (enforced — keep them):

- **No `Shipeasy.configure` call inside snippet code** — configuration is shown on
  the Installation page. A one-line "Assumes `Shipeasy.configure` ran at startup —
  see Installation." note is fine.
- **Construct the bound client on its own line**, with a "construct once per
  callsite" comment — never chain `Shipeasy::Client.new(user).get_flag(...)`.
...
followed by a tight, copy-paste-runnable usage guide that links each section's
full page + snippets (raw Pages URLs) and points at the manifest URL for runtime
discovery. The frontmatter ships with it (a consumer can install it verbatim, and
`shipeasy-skill install` does), so keep it valid.

## The README is generated from these docs

`../README.md` is **generated** by `../scripts/gen_readme.rb` — it pulls the
...
`ruby scripts/gen_readme.rb` (or `rake readme`) from the repo root and commit the
result; CI (`test.yml`, the `readme` job) fails if it drifts. Never hand-edit the
README.

## Working on the docs

- **Keep docs in lockstep with the code.** Any public API/behaviour change in
  `lib/` updates the matching page(s), snippet(s), and the skill in the *same*
  change, then regenerate the README (repo-root `CLAUDE.md` hard rule).
- After edits: `ruby -rjson -e 'JSON.parse(File.read("docs/manifest.json"))'` and
