...
Outside Rails, the engine can emit the loader tag itself. Note this tag carries
the **public client key**, never the server key:

```ruby
Shipeasy.engine.i18n_script_tag(client_key, profile: "{{PROFILE}}")
```

`bootstrap_script_tag` also takes an `i18n_profile:` so the browser hydrates the
same profile (see [advanced](advanced.md)).
