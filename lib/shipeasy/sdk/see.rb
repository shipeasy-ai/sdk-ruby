...
# /collect. `causes_the` and `.to` are two halves of one sentence and must stay
# adjacent, so pass the extras inline on the terminal:
#
#   client.see(e).causes_the("checkout").to("use cached prices")
#   client.see(e).causes_the("checkout").to("use cached prices", { order_id: oid })
#
# `extras` remains a chainable setter callable before `.to`, but do NOT wedge it
# between the subject and the outcome — it splits the consequence in half and is
# hard to read.
#
# `.extras` chained AFTER `.to` is ignored with a warning (the report already
# went out) — it never raises into the rescue block, but its extras are dropped.
#
