  ...
  class APIKeysApi
    ...
    def list_keys_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'q'].nil? && opts[:'q'].to_s.length > 100
        fail ArgumentError, 'invalid value for "opts[:"q"]" when calling APIKeysApi.list_keys, the character length must be smaller than or equal to 100.'
      end
    ...
    def revoke_key_with_http_info(id, opts = {})
      ...
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling APIKeysApi.revoke_key"
      end
      if @api_client.config.client_side_validation && id.to_s.length > 128
        fail ArgumentError, 'invalid value for "id" when calling APIKeysApi.revoke_key, the character length must be smaller than or equal to 128.'
      end

      if @api_client.config.client_side_validation && id.to_s.length < 1
        fail ArgumentError, 'invalid value for "id" when calling APIKeysApi.revoke_key, the character length must be greater than or equal to 1.'
      end
    ...
    # Renames a key or re-notes what it is for. **Only `name` and `description` are editable** — everything that gives the key its authority (`type`, `env`, `scopes`, `expires_at`) is fixed at mint, because the worker derives the read env and permissions from the key itself. To change any of those, rotate: mint a replacement and revoke this one.  Omitted fields are left untouched; send `null` to clear one. Returns the updated key row. Revoked keys can still be relabelled (the audit trail outlives the credential). Returns `404` if no such key exists in the project.  **Use case:** A key minted by `shipeasy setup` carries an auto-composed provenance label — rename it to the service that actually holds it and note the rotation owner.
    # @param id [String] Stable opaque key id (UUID) returned by &#x60;create&#x60; / &#x60;list&#x60;.
    # @param update_key_request [UpdateKeyRequest] 
    # @param [Hash] opts the optional parameters
    # @option opts [String] :x_project_id Project the request operates on. Optional — defaults to the project the SDK key belongs to; pass it only to scope a multi-project key (the generated client sets it once from its configuration, so per-call callers never thread it).
    # @return [KeyRecord]
    def update_key(id, update_key_request, opts = {})
      data, _status_code, _headers = update_key_with_http_info(id, update_key_request, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :x_project_id Project the request operates on. Optional — defaults to the project the SDK key belongs to; pass it only to scope a multi-project key (the generated client sets it once from its configuration, so per-call callers never thread it).
    # @return [Array<(KeyRecord, Integer, Hash)>] KeyRecord data, response status code and response headers
    def update_key_with_http_info(id, update_key_request, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: APIKeysApi.update_key ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling APIKeysApi.update_key"
      end
      if @api_client.config.client_side_validation && id.to_s.length > 128
        fail ArgumentError, 'invalid value for "id" when calling APIKeysApi.update_key, the character length must be smaller than or equal to 128.'
      end

      if @api_client.config.client_side_validation && id.to_s.length < 1
        fail ArgumentError, 'invalid value for "id" when calling APIKeysApi.update_key, the character length must be greater than or equal to 1.'
      end
      ...
      if @api_client.config.client_side_validation && update_key_request.nil?
        fail ArgumentError, "Missing the required parameter 'update_key_request' when calling APIKeysApi.update_key"
      end
      # resource path
      local_var_path = '/api/admin/keys/{id}'.sub('{id}', CGI.escape(id.to_s))
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'KeyRecord'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerSdkKey']

      new_options = opts.merge(
        :operation => :"APIKeysApi.update_key",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: APIKeysApi#update_key\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
