...
module Shipeasy::Admin::Generated
  ...
  class CreateCopilotTriggerRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shipeasy::Admin::Generated::CreateCopilotTriggerRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shipeasy::Admin::Generated::CreateCopilotTriggerRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      provider_validator = EnumAttributeValidator.new('String', ["copilot_trigger"])
    ...
    def provider=(provider)
      validator = EnumAttributeValidator.new('String', ["copilot_trigger"])
    ...
    def hash
      [provider, name, events, config, token, enabled].hash
    end
