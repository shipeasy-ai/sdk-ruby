...
module Shipeasy::Admin::Generated
  ...
  class PushI18nKeysRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        :'profile_id' => :'String',
        :'keys' => :'Array<UpsertI18nKeysRequestKeysInner>',
        :'force' => :'Boolean'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shipeasy::Admin::Generated::PushI18nKeysRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shipeasy::Admin::Generated::PushI18nKeysRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @keys.length > 5000
        invalid_properties.push('invalid value for "keys", number of items must be less than or equal to 5000.')
      end

      if @keys.length < 1
        invalid_properties.push('invalid value for "keys", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @keys.length > 5000
    ...
    def keys=(keys)
      ...
      if keys.length > 5000
        fail ArgumentError, 'invalid value for "keys", number of items must be less than or equal to 5000.'
      end

      if keys.length < 1
        fail ArgumentError, 'invalid value for "keys", number of items must be greater than or equal to 1.'
      end
