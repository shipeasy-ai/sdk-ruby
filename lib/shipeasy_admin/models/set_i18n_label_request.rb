...
module Shipeasy::Admin::Generated
  ...
  class SetI18nLabelRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shipeasy::Admin::Generated::SetI18nLabelRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shipeasy::Admin::Generated::SetI18nLabelRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@profile.nil? && @profile.to_s.length > 64
        invalid_properties.push('invalid value for "profile", the character length must be smaller than or equal to 64.')
      end

      if !@profile.nil? && @profile.to_s.length < 1
        invalid_properties.push('invalid value for "profile", the character length must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@profile.nil? && @profile.to_s.length > 64
      return false if !@profile.nil? && @profile.to_s.length < 1
      true
    end
    ...
    def profile=(profile)
      ...
      if profile.to_s.length > 64
        fail ArgumentError, 'invalid value for "profile", the character length must be smaller than or equal to 64.'
      end

      if profile.to_s.length < 1
        fail ArgumentError, 'invalid value for "profile", the character length must be greater than or equal to 1.'
      end
    ...
    def hash
      [key, value, profile, description].hash
    end
