...
module Shipeasy::Admin::Generated
  ...
  class UpdateCursorTriggerRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shipeasy::Admin::Generated::UpdateCursorTriggerRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shipeasy::Admin::Generated::UpdateCursorTriggerRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@ops_key.nil? && @ops_key.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@ops_key.nil? && @ops_key.to_s.length < 1
      true
    end
    ...
    def hash
      [provider, name, config, api_key, ops_key].hash
    end
