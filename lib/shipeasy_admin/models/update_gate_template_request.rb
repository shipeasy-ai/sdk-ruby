...
module Shipeasy::Admin::Generated
  ...
  class UpdateGateTemplateRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shipeasy::Admin::Generated::UpdateGateTemplateRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shipeasy::Admin::Generated::UpdateGateTemplateRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@name.nil? && @name.to_s.length > 140
    ...
    def valid?
      ...
      return false if !@name.nil? && @name.to_s.length > 140
      ...
      return false if !@rules.nil? && @rules.length < 1
      true
    end
